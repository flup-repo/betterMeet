...
/// Manage betterMeet's LaunchAgent so the daemon starts at login.
///
/// We deliberately do NOT use SMAppService.mainApp here — that requires a full
/// .app bundle. Since betterMeet ships as a single binary in /usr/local/bin, a
...
struct Install: ParsableCommand {
    ...
    @Flag(name: .long, help: "Register betterMeet to start at login.")
    ...
    private static let label = "com.flup-repo.betterMeet"
    ...
    private func writeAgent() throws {
        ...
        let plist: [String: Any] = [
            ...
            "StandardOutPath": "/tmp/betterMeet.out.log",
            "StandardErrorPath": "/tmp/betterMeet.err.log",
        ]
        ...
        print("  binary: \(binary)")
        print("  logs:   /tmp/betterMeet.out.log, /tmp/betterMeet.err.log")
    }
    ...
    private func resolveBinaryPath() throws -> String {
        // /usr/local/bin/betterMeet is the canonical install path. Honor a real
        // location if running from elsewhere (e.g. dev).
        let candidate = "/usr/local/bin/betterMeet"
