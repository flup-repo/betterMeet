...
enum DoctorReport {
    ...
    static func checkMicrophone() -> Check {
        ...
        case .denied, .restricted:
            return Check(
                name: "microphone",
                status: .fail("denied"),
                remediation: "System Settings → Privacy & Security → Microphone → enable for betterMeet (or your terminal)"
            )
        @unknown default:
    ...
    static func checkTranscription() -> Check {
        ...
        let version: AsrModelVersion = (try? Config.transcriptionSettings().model) == .v2 ? .v2 : .v3
